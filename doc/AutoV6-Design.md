...
**AutoV6** 解决这个问题：用户预先为每个 Wi-Fi 配置一条规则，之后切换网络时 App 自动完成配置切换，完全无感知。
...
| 默认行为 | 无匹配规则时保持当前配置不变 |
| 开机自启 | 通过 SMAppService 注册为 Login Item，登录后自动运行 |
...
```
主 App（沙盒）──XPC──▶ Privileged Helper（root 权限，无沙盒）──▶ networksetup
```

由于 App Store 强制沙盒，`networksetup` 命令不能在主 App 内直接调用。采用 **SMJobBless + XPC Privileged Helper** 的合规方案：主 App 负责 UI 和逻辑，Helper 进程以 root 权限运行并执行系统命令。首次安装 Helper 时系统会弹出管理员密码对话框进行授权。
...
│
├── AutoV6Helper/                 # Target B — Privileged Helper（root 权限，无沙盒）
...
└── Resources/
    ...
    ├── Helper-Info.plist       # Helper 的 SMAuthorizedClients 配置
    └── Helper-Launchd.plist    # launchd 注册配置
```
...
> **注意**：macOS 12+ 读取 SSID 需要 **位置服务权限**（`Core Location`），需在 Info.plist 中配置 `NSLocationWhenInUseUsageDescription`，并在首次启动时请求授权。
...
### 菜单栏图标
- 使用 SF Symbols `wifi` 图标 + 当前 IPv6 模式缩写
- 例：`⌁ 自动` / `⌁ 仅本地` / `⌁ 关闭`
...
| Location Services | 读取 Wi-Fi 名称（SSID）所需，需用户授权 |
| Network Information | `com.apple.security.network.client` entitlement |
| XPC Service | 主 App 与 Helper 通信 |
| SMJobBless | 安装 Privileged Helper，首次需管理员密码授权 |

首次安装后，App 依次引导用户：
1. 授权**位置服务**权限（读取 SSID 所需）
2. 输入**管理员密码**安装 Helper（弹出系统授权对话框）
